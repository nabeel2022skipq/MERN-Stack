...
function ActionMovies() {
    ...
    const movies = moviesList.filter(m => m.category === "Action")
        ...
        <React.Fragment>
            <div>
                <div className='mb-5' style={{ borderLeft: "15px solid #FF0000" }}>
                    <p className='text-2xl text-white px-10 py-5 font-extrabold'>Action</p>
                </div>
                        ...
                        {movies.map(m => (
                            <div onMouseOver={() => handleCheckHover(m._ID, m.gifsrc)} onMouseLeave={() => handleCheckHoveraway(m._ID, m.imgsrc)}>
                                <div className={`relative bg-black rounded-lg select-none cursor-pointer hover:transform hover:scale-105 hover:transition hover:ease-in-out hover:duration-700 duration-700 p-2 md:p-1`} id='card' onMouseOver={() => handleMouseOver(m._ID)} onMouseLeave={() => handleMouseLeave(m._ID)}>
                                    <div>
                                        <img id={`image${m._ID}`} className='w-full h-64 ' src={m.imgsrc} alt="actionmovies"></img>
                                    </div>
